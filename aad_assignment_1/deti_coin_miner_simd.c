...
void search_deti_coins_simd(const char *custom_text, u64_t max_attempts) {
    ...
    #pragma omp parallel
    {
        ...
        // Contador de moedas local (mais rápido que 'atomic' no loop)
        u64_t thread_local_coins = 0;
        ...
        // Adiciona as moedas locais ao total global (uma só vez)
        #pragma omp atomic update
        total_coins_found += thread_local_coins;
